-- =====================================================================
-- MARGEN REAL v2 — todo SIN IVA y con TARIFA POR UNIDAD (sin desfase)
-- Ejecuta en Supabase -> SQL Editor. Reemplaza vistas (idempotente).
--
-- Dos arreglos de fondo:
--  (1) Ventas SIN IVA por pais (el IVA repercutido va a Hacienda).
--  (2) Las tarifas de tarjetas y P&L se calculan con la TARIFA REAL POR UNIDAD
--      (v_fee_sku) x unidades vendidas, NO con el settlement por fecha (que
--      lleva desfase: metia tarifas de junio en julio y dejaba los dias de hoy
--      sin tarifa). Asi TODOS los periodos son consistentes.
--  (3) PPC = gasto real de Ads API (ppc_dia).
-- =====================================================================

-- 1) VENTAS DIARIAS SIN IVA
create or replace view v_ventas_dia as
select fecha, sku, sum(uds) as uds,
...
-- 2) VENTAS MENSUALES SIN IVA (grafico de barras)
...
-- 3) TARIFA POR UNIDAD APLICADA POR DIA (fba+com reales por ud x uds vendidas)
--    Sin desfase: la tarifa "viaja" con la venta, no con la liquidacion.
--    Si un SKU no tiene liquidacion aun -> estima 15%+15% sobre la venta.
create or replace view v_tarifa_dia as
select
  v.fecha, v.sku, v.uds, v.ventas,
  case when f.uds_liq > 0 then round(v.uds * f.fba / f.uds_liq, 2) else round(v.ventas * 0.15, 2) end as fba,
  case when f.uds_liq > 0 then round(v.uds * f.com / f.uds_liq, 2) else round(v.ventas * 0.15, 2) end as com
from v_ventas_dia v
left join v_fee_sku f on f.sku = v.sku;

-- 4) TARJETAS Hoy/Ayer/Mes/... con tarifa por unidad + PPC Ads API
...
agg as (
  select r.id, r.nom, r.etiqueta, r.orden,
    ...
    -- tarifas = fba+com por unidad (v_tarifa_dia) + almacenaje/devol/otros (settlement, pequeños)
...
-- 5) P&L DEL MES: fba/com por unidad; alm/dev/otros del settlement; PPC Ads API
...
-- 6) P&L POR PERIODO (mismo criterio)
