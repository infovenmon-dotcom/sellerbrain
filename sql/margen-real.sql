-- =====================================================================
-- MARGEN REAL v3 — Venta CON IVA (como Seller Central) + margen correcto
...
-- 1) VENTAS DIARIAS: ventas = CON IVA (display); ventas_neto = SIN IVA (beneficio)
create or replace view v_ventas_dia as
select
  fecha, sku,
  sum(uds) as uds,
  round(sum(ventas), 2) as ventas,        -- CON IVA (lo que reconoces)
  ...
  round(sum(ventas / (case pais
    ...
    when 'IE' then 1.23 else 1.21 end)), 2) as ventas_neto   -- SIN IVA
...
select
  v.fecha, v.sku, v.uds, v.ventas_neto,
  case when f.uds_liq > 0 then round(v.uds * f.fba / f.uds_liq, 2) else round(v.ventas_neto * 0.15, 2) end as fba,
  case when f.uds_liq > 0 then round(v.uds * f.com / f.uds_liq, 2) else round(v.ventas_neto * 0.15, 2) end as com
from v_ventas_dia v
...
agg as (
  select r.id, r.nom, r.etiqueta, r.orden,
    ...
    coalesce((select sum(ventas_neto) from v_ventas_dia v where v.fecha >= r.ini and v.fecha < r.fin),0) neto,
...
v as (select coalesce(sum(ventas),0) ventas, coalesce(sum(ventas_neto),0) neto from v_ventas_dia, mes where fecha >= mes.ini),
...
select round(v.ventas,2) ventas, round(cogs.prod,2) prod, round(tp.fba,2) fba,
       ...
       round(v.ventas - v.neto, 2) as iva_rep          -- IVA repercutido (a Hacienda)
from v, cogs, tp, s, p;
...
language sql stable as $$
  ...
  v as (select coalesce(sum(ventas),0) ventas, coalesce(sum(ventas_neto),0) neto from v_ventas_dia, rango where fecha >= ini and fecha <= fin),
